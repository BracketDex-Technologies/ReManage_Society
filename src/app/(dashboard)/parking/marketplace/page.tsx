...
export default function ParkingMarketplace() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toastT.error("Failed to load marketplace data");
    } finally {
      setLoading(false);
    }
  }, [toastT]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isOptionalTenDigitPhone(form.contactPhone)) {
      toastT.error("Enter a valid 10-digit contact phone");
      return;
    }
    setSaving(true);
    try {
      ...
      if (res.ok) {
        toastT.success(showModal === "share" ? "Slot listed for sharing!" : "Parking request posted!");
        setShowModal(null);
        ...
        const d = await res.json();
        toastT.error(d.error || "Failed to post");
      }
    } catch {
  ...
  const handleExchangeAction = async (payload: { requestId?: string; sharingId?: string; action: string }) => {
    try {
      ...
      if (!res.ok) {
        toastT.error(data.error || "Could not update parking exchange");
        return;
      }
      toastT.success("Parking exchange updated");
      fetchData();
    } catch {
      toastT.error("Something went wrong");
    }
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-text-primary tracking-tight">{t("Parking Exchange")}</h1>
            <p className="text-sm text-text-secondary mt-1 font-medium">{t("Share assigned slots or request short-term parking from verified residents")}</p>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-primary to-indigo-600 rounded-[2.5rem] p-10 text-white shadow-2xl shadow-primary/20 relative overflow-hidden group">
         ...
         <div className="relative z-10 max-w-lg">
            <h2 className="text-4xl font-black mb-4 leading-tight">{t("Use parking without confusion.")}</h2>
            <p className="text-primary-light font-medium mb-10 opacity-80 leading-relaxed">{t("Residents can share only assigned slots, request parking for guests, and avoid double booking through society rules.")}</p>
            <div className="flex flex-wrap gap-4">
               <button onClick={() => setShowModal("share")} className="btn !bg-white !text-primary border-none px-8 py-4 rounded-2xl font-black shadow-xl hover:scale-105 active:scale-100 transition-all flex items-center gap-2">
                  <Share2 className="w-5 h-5" /> {t("Share Assigned Slot")}
               </button>
               <button onClick={() => setShowModal("request")} className="btn bg-white/20 hover:bg-white/30 border-none text-white backdrop-blur-md px-8 py-4 rounded-2xl font-black flex items-center gap-2" disabled={duesBlocked}>
                  <Search className="w-5 h-5" /> {t("Request Parking")}
               </button>
            </div>
         </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 px-2">
         <div className="flex items-center gap-2 text-xs font-bold text-text-secondary uppercase tracking-[0.15em] bg-surface/50 border border-border/50 px-4 py-2 rounded-full">
            <ShieldCheck className="w-4 h-4 text-emerald-500" /> {t("Secure Exchange")}
         </div>
         <div className="flex items-center gap-2 text-xs font-bold text-text-secondary uppercase tracking-[0.15em] bg-surface/50 border border-border/50 px-4 py-2 rounded-full">
            <MessageSquare className="w-4 h-4 text-primary" /> {t("Direct Chat")}
         </div>
         <div className="flex items-center gap-2 text-xs font-bold text-text-secondary uppercase tracking-[0.15em] bg-surface/50 border border-border/50 px-4 py-2 rounded-full">
            <IndianRupee className="w-4 h-4 text-orange-500" /> {t("Flexible Pricing")}
         </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] border border-border/50 shadow-xl p-8 min-h-[400px]">
         {loading ? (
           <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <p className="text-xs font-black text-text-secondary uppercase tracking-widest">{t("SYNCHRONIZING EXCHANGE...")}</p>
           </div>
           ...
           <div className="text-center py-32 space-y-4">
              ...
              <p className="text-xl font-bold text-text-primary">{t("Nothing listed here yet!")}</p>
              <p className="text-sm text-text-secondary max-w-sm mx-auto">
                {activeTab === "available"
                  ? t("Be the first to share your slot for the upcoming weekend.")
                  : t("Be the first to request parking for the upcoming weekend.")}
              </p>
           </div>
         ) : (
                   ...
                   <div className="bg-white p-6">
                      <div className="flex justify-between items-start mb-6">
                         <div className="flex items-center gap-3">
                            ...
                            <div>
                               <p className="text-xs font-black text-text-tertiary uppercase tracking-widest">
                                 {activeTab === "available" ? t("Shared By") : t("Requested By")}
                               </p>
                               <p className="text-sm font-bold text-text-primary">
                                 {item.ownerOccupancy?.person?.name || (item.flat || item.requesterFlat)?.ownerName || t("Verified resident")}
                               </p>
                               <p className="text-[10px] text-text-secondary mt-0.5">
                                 {t("Flat")} {item.ownerOccupancy?.unit?.flatNumber || (item.flat || item.requesterFlat)?.flatNumber || "-"}
                               </p>
                            </div>
                         </div>
                         <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${item.isPaid ? "bg-orange-50 text-orange-600" : "bg-emerald-50 text-emerald-600"}`}>
                            {item.isPaid ? `₹ ${item.price}` : t("Free Share")}
                         </div>
                      </div>
                      ...
                      <p className="text-sm text-text-secondary leading-relaxed line-clamp-2 px-1 mb-8 italic">
                         &ldquo;{item.description || item.purpose || t("No additional details provided.")}&rdquo;
                      </p>
                          ...
                          <div className="space-y-2">
                            {item.requests.map((request) => (
                              <div key={request.id} className="flex items-center justify-between gap-2">
                                <span className="text-xs font-bold text-text-primary">{t("Flat")} {request.requesterFlat?.flatNumber || "-"}</span>
                                <div className="flex gap-1">
                                  <button type="button" onClick={() => handleExchangeAction({ requestId: request.id, action: "accept" })} className="px-2 py-1 rounded-lg bg-success-bg text-success text-[10px] font-black">{t("Accept")}</button>
                                  <button type="button" onClick={() => handleExchangeAction({ requestId: request.id, action: "reject" })} className="px-2 py-1 rounded-lg bg-danger/10 text-danger text-[10px] font-black">{t("Reject")}</button>
                                </div>
                              </div>
                            ))}
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          {item.canManage && (
                            <button onClick={() => openEdit(item, "share")} className="text-[10px] font-black text-primary hover:underline uppercase flex items-center justify-center gap-1">
                              <Pencil className="w-3 h-3" /> {t("Edit Listing")}
                            </button>
                          )}
                          {item.canManage && (
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          {item.canManage && (
                            <button onClick={() => openEdit(item, "request")} className="text-[10px] font-black text-primary hover:underline uppercase flex items-center justify-center gap-1">
                              <Pencil className="w-3 h-3" /> {t("Edit Request")}
                            </button>
                          )}
                          {item.canManage && (
          ...
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] !p-10 shadow-2xl animate-in zoom-in-95 duration-300 relative overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-4 mb-10">
                 ...
                 <div>
                    <h3 className="text-3xl font-black text-text-primary tracking-tighter">
                      {editing ? t("Edit Parking Window") : showModal === "share" ? t("Share Assigned Slot") : selectedShare ? t("Request This Slot") : t("Request Parking")}
                    </h3>
                    <p className="text-sm font-medium text-text-secondary mt-1">
                      {showModal === "share" ? t("Only your active assigned slot can be shared") : t("Committee can audit every request and response")}
                    </p>
                 </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-text-tertiary ml-1">{t("From Date")}</label>
                      <input 
                        type="date" 
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-text-tertiary ml-1">{t("Till Date")}</label>
                      <input
                        type="date"
                        min={form.date}
                        className="input !rounded-2xl !bg-surface/50 !border-border/60 p-4 font-black"
                   ...
                   <div className="grid grid-cols-2 gap-4 md:col-span-2">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-text-tertiary ml-1">{t("From")}</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-text-tertiary ml-1">{t("Till")}</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-text-tertiary ml-1">{t("Contact Number")}</label>
                  <input
                    {...phoneInputProps}
                    className="input !rounded-2xl !bg-surface/50 !border-border/60 p-4 font-black"
                    placeholder={t("Mobile number for coordination")}
                    value={form.contactPhone}
                    onChange={(e) => setForm({ ...form, contactPhone: sanitizePhoneInput(e.target.value) })}
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-text-tertiary ml-1">
                    {showModal === 'share' ? t('Notes for users') : t('Why do you need this?')}
                  </label>
                  <textarea 
                    className="input !rounded-2xl !bg-surface/50 !border-border/60 p-4 min-h-[120px] font-medium resize-none" 
                    placeholder={showModal === 'share' ? t("e.g. My car is being serviced so slot is empty all day.") : t("e.g. My cousin is visiting for dinner with his SUV.")}
                ...
                <div className="flex items-center justify-between bg-surface/50 p-6 rounded-3xl border border-border/40">
                   <div className="flex items-center gap-3">
                      ...
                      />
                      <label htmlFor="isPaid" className="text-sm font-bold text-text-primary cursor-pointer">{t("Paid Transaction")}</label>
                   </div>
                   {form.isPaid && (
                ...
                <div className="flex gap-4 pt-4">
                  <button type="button" onClick={closeModal} className="flex-1 btn btn-secondary !rounded-[1.25rem] py-4 font-black">{t("Discard")}</button>
                  <button type="submit" disabled={saving || (duesBlocked && showModal === "request")} className="flex-[2] btn btn-primary !rounded-[1.25rem] py-4 font-black shadow-xl shadow-primary/30">
                    {saving ? t("Saving...") : editing ? t("Save Changes") : showModal === 'share' ? t("List Available Slot") : t("Post Request")}
                  </button>
                </div>
              </form>
            </div>
