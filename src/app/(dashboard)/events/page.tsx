...
export default function EventsPage() {
  ...
  const rsvp = async(eventId:string, response:string)=>{ try{ const r=await fetch("/api/events",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({eventId,response})}); if(r.ok){toast.success(response==="attending"?"RSVP confirmed":"RSVP updated");fetch_();}else{const d=await r.json();toast.error(d.error||"Failed")} }catch{toast.error("Error")} };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-500 px-4 sm:px-6 lg:px-0 pb-20">
      <ModulePageHeader
        ...
        description="Plan society programs, view schedules, and manage RSVPs."
      ...
      {loading?<div className="flex items-center justify-center py-20"><div className="spinner !w-8 !h-8" /></div>:events.length===0?(
        <ModuleEmptyState icon={Calendar} title="No events scheduled" description="Create your first society event to open RSVPs." tone="emerald" />
      ):(
        <div className="space-y-6">
          {upcoming.length>0&&<div className="space-y-3">
            <ModuleSectionTitle title="Upcoming" description="Confirmed and open event slots." />
          ...
          {past.length>0&&<div className="space-y-3">
            <ModuleSectionTitle title="Past Events" description="Recently completed society programs." />
